...
impl Value {
    ...
    /// Creates a Value from slice of bytes and other parameters.
    /// It returns `Value::Invalid` on invalid arguments.
...
mod tests {
    ...
    fn test_value_from_parts() {
        ...
        let tt = [
            ...
            Case {
                buf: [1i8, 2i8].iter().flat_map(|v| v.to_le_bytes()).collect(),
                array: true,
                base_type: FitBaseType::SINT8,
                arch: 0,
            ...
            Case {
                buf: [1i8, 2i8].iter().flat_map(|v| v.to_be_bytes()).collect(),
                array: true,
                base_type: FitBaseType::SINT8,
                arch: 1,
                expected: Value::VecInt8(vec![1, 2]),

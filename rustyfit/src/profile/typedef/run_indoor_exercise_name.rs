...
impl fmt::Display for RunIndoorExerciseName {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.as_str() {
            Some(s) => write!(f, "{}", s),
            None => write!(f, "RunIndoorExerciseName({})", self.0),
        }
    }
}
...
impl Serialize for RunIndoorExerciseName {
    ...
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("RunIndoorExerciseName", 2)?;
...
#[cfg(feature = "serde")]
impl<'de> Deserialize<'de> for RunIndoorExerciseName {

...
impl fmt::Display for SledgeHammerExerciseName {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.as_str() {
            Some(s) => write!(f, "{}", s),
            None => write!(f, "SledgeHammerExerciseName({})", self.0),
        }
    }
}
...
impl Serialize for SledgeHammerExerciseName {
    ...
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("SledgeHammerExerciseName", 2)?;
...
#[cfg(feature = "serde")]
impl<'de> Deserialize<'de> for SledgeHammerExerciseName {

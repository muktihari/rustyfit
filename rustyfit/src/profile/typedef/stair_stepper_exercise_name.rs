...
impl fmt::Display for StairStepperExerciseName {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.as_str() {
            Some(s) => write!(f, "{}", s),
            None => write!(f, "StairStepperExerciseName({})", self.0),
        }
    }
}
...
impl Serialize for StairStepperExerciseName {
    ...
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("StairStepperExerciseName", 2)?;
...
#[cfg(feature = "serde")]
impl<'de> Deserialize<'de> for StairStepperExerciseName {

...
impl fmt::Display for BikeOutdoorExerciseName {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.as_str() {
            Some(s) => write!(f, "{}", s),
            None => write!(f, "BikeOutdoorExerciseName({})", self.0),
        }
    }
}
...
impl Serialize for BikeOutdoorExerciseName {
    ...
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("BikeOutdoorExerciseName", 2)?;
...
#[cfg(feature = "serde")]
impl<'de> Deserialize<'de> for BikeOutdoorExerciseName {

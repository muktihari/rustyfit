...
impl fmt::Display for IndoorRowExerciseName {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.as_str() {
            Some(s) => write!(f, "{}", s),
            None => write!(f, "IndoorRowExerciseName({})", self.0),
        }
    }
}
...
impl Serialize for IndoorRowExerciseName {
    ...
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("IndoorRowExerciseName", 2)?;
...
#[cfg(feature = "serde")]
impl<'de> Deserialize<'de> for IndoorRowExerciseName {
